...
CHARACTER_NAMES = json.loads((ROOT / "site/character-names.json").read_text())
...
def character_display_text(text):
    return re.sub(
        r"(^|[^A-Za-z0-9_./?=&%#-])(?:(?:Rubber|RUBBER)[ \t]+)?(?:Ducky|DUCKY)"
        r"(?![A-Za-z0-9_/-]|\.(?:FCStd|stl|step|3mf|blend|png|jpg|mp4|json|gz|zip)\b)",
        lambda match: match[1] + CHARACTER_NAMES["ducky"], text)
...
class Renderer(HTMLParser):
    ...
    def translate(self, text):
        ...
        if not JP.search(key) and key != "。":
            return character_display_text(text)
        if key not in self.messages:
            raise ValueError(f"Untranslated template: {self.key}: {key}")
        translated = self.messages[key] if self.locale == "en" else key
        return (" " if text[:1].isspace() else "") + character_display_text(translated) + (" " if text[-1:].isspace() else "")
    ...
    def handle_starttag(self, tag, attrs):
        ...
        for attribute, value in attrs:
            ...
            elif value is not None and (attribute in ATTRIBUTES or (tag == "meta" and attribute == "content")):
                value = character_display_text(value)
    ...
    def handle_data(self, text):
        if self.skip or "code" in self.stack:
        ...
        if JP.search(text) or normalize(text) == "。":
            ...
            self.result.append(escape(character_display_text(text)))
...
def main():
    ...
    source_ids = [identifier(key) for key in messages]
    if len(set(source_ids)) != len(source_ids):
        raise ValueError("Translation source identifiers collide")
    payload = {"routes": ROUTES, "docs": DOCS, "characterNames": CHARACTER_NAMES}
    ...
    (ROOT / "assets/translations.js").write_text(
        ...
        + "const messages=" + compact_json(messages) + ";\n"
        + "const sources=Object.keys(messages);\n"
        + "const sourceIds=Object.fromEntries(" + compact_json(source_ids)
        + ".map((id,index)=>[id,sources[index]]));\n"
        + "export default {messages,sourceIds,..." + compact_json(payload) + "};\n")
